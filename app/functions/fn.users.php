...
function fn_recover_password_login($ekey = null)
{
    ...
    if ($ekey) {
        $u_id = db_get_field("SELECT object_id FROM ?:ekeys WHERE ekey = ?s AND object_type = 'U' AND ttl > ?i", $ekey, TIME);
    ...
    if ($user_email) {
        ...
        if (!empty($u_data['email'])) {
            ...
            if ($notify) {
                ...
                $result = array('company_id' => $u_data['company_id'], 'key' => $_data['ekey'], 'user_type' => $u_data['user_type']);
            }
