...
function fn_ult_get_products(&$params, &$fields, &$sortings, &$condition, &$join, &$sorting, &$group_by, &$lang_code)
{
    if (Registry::get('runtime.company_id')) {
        ...
        if (in_array('product_name', $params['extend'])) {
            ...
            $fields[] = 'IF(shared_descr.product_id IS NOT NULL, shared_descr.promo_text, descr1.promo_text) as promo_text';
...
function fn_ult_url_post(&$result_url, &$area, &$url, &$prefix, &$company_id_in_url, &$lang_code)
{
    ...
    if (isset($company_id) && ($prefix == 'https' || $prefix == 'http' || $prefix == 'current')) {
        if ($area == 'C') { // Build link to the frontend in the backend
            ...
            if (empty($storefronts[$company_id])) {
                $storefronts[$company_id] = db_get_row('SELECT storefront, secure_storefront FROM ?:companies WHERE company_id = ?i', $company_id);
            }

            $storefront = $storefronts[$company_id][$storefront_field];

            $location = $protocol == 'https' ? Registry::get('config.https_location') : Registry::get('config.http_location');

            $result_url = str_replace($location, $protocol . '://' . $storefront, $result_url);
...
function fn_init_store_params_by_host(&$request, $area = AREA)
{
    ...
    if (!empty($request['switch_company_id']) && $request['switch_company_id'] != 'all' && !isset($request['skip_config_changing'])) { // theme for company with id = 0 cannot be loaded.
        $company_data = db_get_row('SELECT storefront, secure_storefront, redirect_customer FROM ?:companies WHERE company_id = ?i', $request['switch_company_id']);
